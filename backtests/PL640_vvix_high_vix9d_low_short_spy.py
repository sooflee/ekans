"""PL640_vvix_high_vix9d_low_short_spy — VVIX High + VIX9D < VIX -> Short SPY (Convexity Buying Precedes Vol Spike)"""
import sys
...
def main():
    sid = "PL640_vvix_high_vix9d_low_short_spy"
    tickers = ["SPY", "^VVIX", "^VIX", "^VIX9D"]
    try:
    ...
    m = compute_metrics(pnl, benchmark=spy_r, name="VVIX High + VIX9D < VIX -> Short SPY (Convexity Buying Precedes Vol Spike)", positions=pos)
    save_result(sid, m, extra={
        "rule": "Use yfinance ^VVIX, ^VIX, ^VIX9D daily closes. ENTER short SPY when (a) ^VVIX close is in the top 10th percentile of trailing 252 trading-day distribution AND (b) ^VIX9D / ^VIX <= 0.95 (calm front-end",
        "mechanism": "Use yfinance ^VVIX, ^VIX, ^VIX9D daily closes. ENTER short SPY when (a) ^VVIX close is in the top 10th percentile of trailing 252 trading-day distribution AND (b) ^VIX9D / ^VIX <= 0.95 (calm front-end relative to 30-day VIX). HOLD 10 trading days (~2 weeks). EXIT early on (i) SPY down >-3% (profit t",
