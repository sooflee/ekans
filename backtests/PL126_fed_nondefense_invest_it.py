"""PL126_fed_nondefense_invest_it -- Fed Nondefense Investment -> Long ACN+LDOS
...
def main():
    sid = "PL126_fed_nondefense_invest_it"
    
    # Load FRED data
    try:
        fred = load_fred(['A782RX1Q020SBEA'], start="1990-01-01")
    ...
    series = fred["A782RX1Q020SBEA"].dropna()
    if len(series) < 13:
    ...
    # Load prices
    try:
        px = load_prices(['ACN', 'LDOS', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Fed Nondefense Investment -> Long ACN+LDOS")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long ACN+LDOS 252d when A782RX1Q020SBEA YoY > +5% for 2 quarters",
        "source": "FRED A782RX1Q020SBEA; yfinance",
