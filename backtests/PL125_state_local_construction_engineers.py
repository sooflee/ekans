"""PL125_state_local_construction_engineers -- State/Local Construction Surge -> Long PWR+MTZ
Long PWR+MTZ 252d when TLPBLCONS YoY > +10% for 3 months
"""
import sys
...
def main():
    sid = "PL125_state_local_construction_engineers"
    
    # Load FRED data
    try:
        fred = load_fred(['TLPBLCONS'], start="1990-01-01")
    ...
    # Load prices
    try:
        px = load_prices(['PWR', 'MTZ', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="State/Local Construction Surge -> Long PWR+MTZ")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long PWR+MTZ 252d when TLPBLCONS YoY > +10% for 3 months",
        "source": "FRED TLPBLCONS; yfinance",
