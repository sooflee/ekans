"""PL654_pjm_bra_long_vst_nrg_ceg — PJM BRA Auction Calendar Anchor -> Long VST+NRG+CEG IPP Basket"""
import sys
...
def main():
    sid = "PL654_pjm_bra_long_vst_nrg_ceg"
    tickers = ["VST", "NRG", "CEG", "TLN", "SPY"]
    try:
    ...
    m = compute_metrics(pnl, benchmark=spy_r, name="PJM BRA Auction Calendar Anchor -> Long VST+NRG+CEG IPP Basket", positions=pos)
    save_result(sid, m, extra={
        "rule": "Use a calendar-based event anchor for the PJM Base Residual Auction. PJM BRA results typically post in late July of each year. Backtestable approximation: ENTER long equal-weight basket (VST, NRG, CEG",
        "mechanism": "Use a calendar-based event anchor for the PJM Base Residual Auction. PJM BRA results typically post in late July of each year. Backtestable approximation: ENTER long equal-weight basket (VST, NRG, CEG) on the first trading day on/after July 28 each year IF (a) FRED ELECPRIIDXM (electricity producer ",
