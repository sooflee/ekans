"""PL112_vehicle_age_auto_parts -- Vehicle Sales Drought -> Long AZO+ORLY
...
def main():
    sid = "PL112_vehicle_age_auto_parts"
    
    # Load FRED data
    try:
        fred = load_fred(['TOTALSA'], start="1990-01-01")
    ...
    series = fred["TOTALSA"].dropna()
    if len(series) < 13:
        return mark_failed(sid, "insufficient data")
    if 12 > 1:
        series = series.rolling(12).mean().dropna()
    ...
    # Load prices
    try:
        px = load_prices(['AZO', 'ORLY', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Vehicle Sales Drought -> Long AZO+ORLY")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long AZO+ORLY 252d when TOTALSA 12mo avg < 14M SAAR for 6 months",
        "source": "FRED TOTALSA; yfinance",
