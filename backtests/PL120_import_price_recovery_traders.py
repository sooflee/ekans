"""PL120_import_price_recovery_traders -- Import Price Recovery -> Long ADM+BG
Long ADM+BG 126d when IR YoY turns positive after 6+ months negative
"""
import sys
...
def main():
    sid = "PL120_import_price_recovery_traders"
    
    # Load FRED data
    try:
        fred = load_fred(['IR'], start="1990-01-01")
    ...
    # Load prices
    try:
        px = load_prices(['ADM', 'BG', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Import Price Recovery -> Long ADM+BG")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long ADM+BG 126d when IR YoY turns positive after 6+ months negative",
        "source": "FRED IR; yfinance",
