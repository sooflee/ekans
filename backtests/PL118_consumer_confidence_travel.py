"""PL118_consumer_confidence_travel -- CB Present Situation Surge -> Long MAR+EXPE
Long MAR+EXPE 126d when CSCICP03USM665S YoY change > +10 for 3 months
"""
import sys
...
def main():
    sid = "PL118_consumer_confidence_travel"
    
    # Load FRED data
    try:
        fred = load_fred(['CSCICP03USM665S'], start="1990-01-01")
    ...
    series = fred["CSCICP03USM665S"].dropna()
    if len(series) < 13:
    ...
    # Load prices
    try:
        px = load_prices(['MAR', 'EXPE', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="CB Present Situation Surge -> Long MAR+EXPE")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long MAR+EXPE 126d when CSCICP03USM665S YoY change > +10 for 3 months",
        "source": "FRED CSCICP03USM665S; yfinance",
