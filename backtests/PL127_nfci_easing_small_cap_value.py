"""PL127_nfci_easing_small_cap_value -- NFCI Easing From Tight -> Long IWN
...
def main():
    sid = "PL127_nfci_easing_small_cap_value"
    
    # Load FRED data
    try:
        fred = load_fred(['NFCI'], start="1990-01-01")
    ...
    series = fred["NFCI"].dropna()
    
    trigger_dates = []
    above_count = 0
    fired = False
    for i in range(len(series)):
        if series.iloc[i] > 0.5:
            above_count += 1
            fired = False
        elif above_count >= 3 and series.iloc[i] < 0 and not fired:
    ...
    # Load prices
    try:
        px = load_prices(['IWN', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="NFCI Easing From Tight -> Long IWN")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long IWN 252d when NFCI drops below 0 after 3+ months above +0.5",
        "source": "FRED NFCI; yfinance",
