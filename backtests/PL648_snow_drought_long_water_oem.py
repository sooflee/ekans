"""PL648_snow_drought_long_water_oem — Western Drought Regime Proxy -> Long WMS+AOS+XYL Water OEM Basket"""
import sys
...
def main():
    sid = "PL648_snow_drought_long_water_oem"
    tickers = ["WMS", "AOS", "XYL", "ROL", "SPY"]
    try:
    ...
    m = compute_metrics(pnl, benchmark=spy_r, name="Western Drought Regime Proxy -> Long WMS+AOS+XYL Water OEM Basket", positions=pos)
    save_result(sid, m, extra={
        "rule": "Approximate the NRCS SNOTEL Upper Colorado Basin SWE deficit signal with a drought-regime proxy. Use FRED USDM-D2 (US Drought Monitor \u2014 D2 or worse coverage % for the Western US) IF available; alterna",
        "mechanism": "Approximate the NRCS SNOTEL Upper Colorado Basin SWE deficit signal with a drought-regime proxy. Use FRED USDM-D2 (US Drought Monitor \u2014 D2 or worse coverage % for the Western US) IF available; alternatively use the price-regime approach: ENTER long equal-weight basket (WMS, AOS, XYL) on the first tr",
