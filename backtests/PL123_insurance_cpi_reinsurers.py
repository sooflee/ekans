"""PL123_insurance_cpi_reinsurers -- Insurance CPI Acceleration -> Long RNR+ACGL
Long RNR+ACGL 252d when CUSR0000SEHC YoY > +8% for 6 months
"""
import sys
...
def main():
    sid = "PL123_insurance_cpi_reinsurers"
    
    # Load FRED data
    try:
        fred = load_fred(['CUSR0000SEHC'], start="1990-01-01")
    ...
    series = fred["CUSR0000SEHC"].dropna()
    if len(series) < 13:
    ...
    # Load prices
    try:
        px = load_prices(['RNR', 'ACGL', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Insurance CPI Acceleration -> Long RNR+ACGL")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long RNR+ACGL 252d when CUSR0000SEHC YoY > +8% for 6 months",
        "source": "FRED CUSR0000SEHC; yfinance",
