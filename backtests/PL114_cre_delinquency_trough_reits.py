"""PL114_cre_delinquency_trough_reits -- CRE Delinquency Trough -> Long BXP+SPG
...
def main():
    sid = "PL114_cre_delinquency_trough_reits"
    
    # Load FRED data
    try:
        fred = load_fred(['DRCLACBS'], start="1990-01-01")
    ...
    series = fred["DRCLACBS"].dropna()
    if len(series) < 4:
        return mark_failed(sid, "insufficient data")
    
    trigger_dates = []
    for i in range(3, len(series)):
        is_declining = all(series.iloc[i-j] < series.iloc[i-j-1] for j in range(2))
        if is_declining and series.iloc[i - 2] > series.iloc[i - 2 - 1]:
    ...
    # Load prices
    try:
        px = load_prices(['BXP', 'SPG', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="CRE Delinquency Trough -> Long BXP+SPG")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long BXP+SPG 252d when DRCLACBS peaks then declines 2 quarters",
