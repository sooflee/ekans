"""PL124_defense_spending_primes -- Defense Spending Acceleration -> Long LMT+RTX+NOC
...
def main():
    sid = "PL124_defense_spending_primes"
    
    # Load FRED data
    try:
        fred = load_fred(['FDEFX'], start="1990-01-01")
    ...
    # Load prices
    try:
        px = load_prices(['LMT', 'RTX', 'NOC', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Defense Spending Acceleration -> Long LMT+RTX+NOC")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long LMT+RTX+NOC 252d when FDEFX YoY > +5% for 2 quarters",
        "source": "FRED FDEFX; yfinance",
