"""PL107_semi_ppi_deflation_trough -- Semi PPI Deflation Trough -> Long SMH
...
def main():
    sid = "PL107_semi_ppi_deflation_trough"
    
    # Load FRED data
    try:
        fred = load_fred(['PCU33443344'], start="1990-01-01")
    ...
    series = fred["PCU33443344"].dropna()
    if len(series) < 13:
    ...
    # Load prices
    try:
        px = load_prices(['SMH', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Semi PPI Deflation Trough -> Long SMH")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long SMH 126d when PCU33443344 YoY turns positive after 6+ months negative",
        "source": "FRED PCU33443344; yfinance",
