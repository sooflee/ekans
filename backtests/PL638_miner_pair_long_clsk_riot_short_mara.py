"""PL638_miner_pair_long_clsk_riot_short_mara — BTC Miner Power-Cost Proxy -> Long CLSK+RIOT / Short MARA"""
import sys
...
def main():
    sid = "PL638_miner_pair_long_clsk_riot_short_mara"
    tickers = ["CLSK", "RIOT", "MARA", "BTC-USD", "SPY"]
    try:
    ...
    m = compute_metrics(pnl, benchmark=spy_r, name="BTC Miner Power-Cost Proxy -> Long CLSK+RIOT / Short MARA", positions=pos)
    save_result(sid, m, extra={
        "rule": "Approximate ERCOT-PJM power-price divergence with stock-relative-strength proxy among publicly traded miners. ENTER long pair (long equal-weight CLSK+RIOT / short MARA) when (a) (CLSK+RIOT 30-day aver",
        "mechanism": "Approximate ERCOT-PJM power-price divergence with stock-relative-strength proxy among publicly traded miners. ENTER long pair (long equal-weight CLSK+RIOT / short MARA) when (a) (CLSK+RIOT 30-day average return) - MARA 30-day return >= +0.05 (TX-heavy miners outperforming MARA, consistent with ERCOT",
