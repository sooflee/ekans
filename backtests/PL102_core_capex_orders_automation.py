...
def main():
    ...
    try:
        px = load_prices(["ROK", "ETN", "AME", "SPY"], start="1998-01-01")
    except Exception as e:
        return mark_failed(sid, f"price: {e}")
    ret = daily_returns(px)
    available = [t for t in ["ROK", "ETN", "AME"] if t in ret.columns]
    if not available:
    ...
    for td in triggers:
        ...
        evts.append({"trigger_date": str(td.date()), "basket_return": round(cr, 4),
                     "spy_return": round(sc, 4) if sc else None})
    if not evts:
        return mark_failed(sid, "no valid events")
    ip = pnl[pnl != 0]
    if len(ip) < 30:
        return mark_failed(sid, f"insufficient days ({len(ip)})")
    m = compute_metrics(ip, benchmark=spy_r, name="Core Capex A34SNO Turn → Long ROK+ETN+AME")
    ra = [e["basket_return"] for e in evts]
    save_result(sid, m, extra={
        "rule": "Long ROK+ETN+AME 126d when A34SNO 3mo MA YoY turns positive after 6+ months negative",
