"""PL109_health_employment_hospitals -- Health Employment Surge -> Long HCA+THC
Long HCA+THC 126d when CES6562000101 YoY > +3% for 3 months
"""
import sys
...
def main():
    sid = "PL109_health_employment_hospitals"
    
    # Load FRED data
    try:
        fred = load_fred(['CES6562000101'], start="1990-01-01")
    ...
    series = fred["CES6562000101"].dropna()
    if len(series) < 13:
    ...
    # Load prices
    try:
        px = load_prices(['HCA', 'THC', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Health Employment Surge -> Long HCA+THC")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long HCA+THC 126d when CES6562000101 YoY > +3% for 3 months",
        "source": "FRED CES6562000101; yfinance",
