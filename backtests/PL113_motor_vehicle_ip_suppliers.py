"""PL113_motor_vehicle_ip_suppliers -- Motor Vehicle IP Recovery -> Long BWA+LEA
...
def main():
    sid = "PL113_motor_vehicle_ip_suppliers"
    
    # Load FRED data
    try:
        fred = load_fred(['IPG3361T3S'], start="1990-01-01")
    ...
    # Load prices
    try:
        px = load_prices(['BWA', 'LEA', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Motor Vehicle IP Recovery -> Long BWA+LEA")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long BWA+LEA 126d when IPG3361T3S YoY turns positive after 6+ months negative",
        "source": "FRED IPG3361T3S; yfinance",
