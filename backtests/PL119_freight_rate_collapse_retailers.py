"""PL119_freight_rate_collapse_retailers -- Import Price Collapse -> Long COST+TGT
...
def main():
    sid = "PL119_freight_rate_collapse_retailers"
    ...
    # Load prices
    try:
        px = load_prices(['COST', 'TGT', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Import Price Collapse -> Long COST+TGT")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long COST+TGT 126d when IR (import prices) YoY < -5% for 3 months",
