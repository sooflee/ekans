"""PL647_ba_cadence_short_ba_long_rtx — BA Execution Stress Proxy -> Short BA / Long RTX Pair"""
import sys
...
def main():
    sid = "PL647_ba_cadence_short_ba_long_rtx"
    tickers = ["BA", "RTX", "LMT", "SPY"]
    try:
    ...
    m = compute_metrics(pnl, benchmark=spy_r, name="BA Execution Stress Proxy -> Short BA / Long RTX Pair", positions=pos)
    save_result(sid, m, extra={
        "rule": "Backtestable proxy for Boeing delivery-cadence guidance misses: when BA has materially underperformed defense peers over 60 trading days AND BA is in a downtrend below its 200d SMA, market is pricing ",
        "mechanism": "Backtestable proxy for Boeing delivery-cadence guidance misses: when BA has materially underperformed defense peers over 60 trading days AND BA is in a downtrend below its 200d SMA, market is pricing execution stress that historically extends. ENTER short BA / long RTX pair when (a) BA 60d return - ",
