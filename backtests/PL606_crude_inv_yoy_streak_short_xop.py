"""PL606_crude_inv_yoy_streak_short_xop — EIA Crude Inv YoY Build Streak + WTI Near 52w High -> Short XOP"""
import sys
...
def main():
    sid = "PL606_crude_inv_yoy_streak_short_xop"
    tickers = ["XOP", "XLE", "CL=F", "SPY"]
    try:
    ...
    m = compute_metrics(pnl, benchmark=spy_r, name="EIA Crude Inv YoY Build Streak + WTI Near 52w High -> Short XOP", positions=pos)
    save_result(sid, m, extra={
        "rule": "Use FRED WCESTUS1 (weekly U.S. ending stocks of crude oil ex-SPR, Wednesday print). Compute YoY change = WCESTUS1(t) / WCESTUS1(t-52 weeks) - 1. STREAK condition: YoY > 0 for 4 consecutive weekly prin",
        "mechanism": "Use FRED WCESTUS1 (weekly U.S. ending stocks of crude oil ex-SPR, Wednesday print). Compute YoY change = WCESTUS1(t) / WCESTUS1(t-52 weeks) - 1. STREAK condition: YoY > 0 for 4 consecutive weekly prints. WTI condition: CL=F front-month close within 5% of trailing 252 trading-day high. ENTER short XO",
