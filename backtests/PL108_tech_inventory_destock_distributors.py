"""PL108_tech_inventory_destock_distributors -- Computer Inventory Destock -> Long ARW+AVT
Long ARW+AVT 126d when A33SNO YoY turns positive after 6+ months decline
"""
import sys
...
def main():
    sid = "PL108_tech_inventory_destock_distributors"
    
    # Load FRED data
    try:
        fred = load_fred(['A33SNO'], start="1990-01-01")
    ...
    # Load prices
    try:
        px = load_prices(['ARW', 'AVT', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Computer Inventory Destock -> Long ARW+AVT")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long ARW+AVT 126d when A33SNO YoY turns positive after 6+ months decline",
        "source": "FRED A33SNO; yfinance",
