"""PL129_term_premium_positive_xlf -- Term Premium Positive -> Long XLF
...
def main():
    sid = "PL129_term_premium_positive_xlf"
    
    # Load FRED data
    try:
        fred = load_fred(['THREEFYTP10'], start="1990-01-01")
    ...
    series = fred["THREEFYTP10"].dropna()
    ...
    for i in range(len(series)):
        if series.iloc[i] < 0:
            below_count += 1
            fired = False
        elif below_count >= 6 and series.iloc[i] >= 0 and not fired:
    ...
    # Load prices
    try:
        px = load_prices(['XLF', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Term Premium Positive -> Long XLF")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long XLF 252d when THREEFYTP10 crosses above 0 after 6+ months below",
        "source": "FRED THREEFYTP10; yfinance",
