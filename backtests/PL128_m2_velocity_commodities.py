"""PL128_m2_velocity_commodities -- M2 Velocity Inflection -> Long DJP
...
def main():
    sid = "PL128_m2_velocity_commodities"
    
    # Load FRED data
    try:
        fred = load_fred(['M2V'], start="1990-01-01")
    ...
    for i in range(len(yoy)):
        if yoy.iloc[i] < 0:
            neg_count += 1
            fired = False
        elif yoy.iloc[i] >= 0 and neg_count >= 4 and not fired:
    ...
    # Load prices
    try:
        px = load_prices(['DJP', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="M2 Velocity Inflection -> Long DJP")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long DJP 252d when M2V YoY turns positive after 4+ quarters decline",
        "source": "FRED M2V; yfinance",
