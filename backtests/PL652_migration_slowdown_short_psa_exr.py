"""PL652_migration_slowdown_short_psa_exr — Existing Home Sales Slowdown Proxy -> Short PSA+EXR Self-Storage"""
import sys
...
def main():
    sid = "PL652_migration_slowdown_short_psa_exr"
    tickers = ["PSA", "EXR", "XLRE", "SPY"]
    try:
    ...
    m = compute_metrics(pnl, benchmark=spy_r, name="Existing Home Sales Slowdown Proxy -> Short PSA+EXR Self-Storage", positions=pos)
    save_result(sid, m, extra={
        "rule": "Approximate USPS COA / Census migration slowdown with FRED EXHOSLUSM495S (Existing Home Sales, monthly SAAR). When existing home sales slow materially, household-relocation life events drop and self-s",
        "mechanism": "Approximate USPS COA / Census migration slowdown with FRED EXHOSLUSM495S (Existing Home Sales, monthly SAAR). When existing home sales slow materially, household-relocation life events drop and self-storage move-in velocity follows with a 1-2Q lag. ENTER short equal-weight basket (PSA, EXR) vs long ",
