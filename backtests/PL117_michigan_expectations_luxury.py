"""PL117_michigan_expectations_luxury -- Michigan Expectations Trough -> Long RL+TPR
...
def main():
    sid = "PL117_michigan_expectations_luxury"
    
    # Load FRED data
    try:
        fred = load_fred(['MICH'], start="1990-01-01")
    ...
    series = fred["MICH"].dropna()
    
    trigger_dates = []
    for i in range(4, len(series)):
        # Check if series[i-bounce] was a local min below 60
        if series.iloc[i - 3] < 60:
            is_trough = (series.iloc[i - 3] <= series.iloc[i - 3 - 1])
            is_bouncing = all(series.iloc[i - j] > series.iloc[i - 3] for j in range(3))
    ...
    # Load prices
    try:
        px = load_prices(['RL', 'TPR', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Michigan Expectations Trough -> Long RL+TPR")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long RL+TPR 126d when MICH hits trough below 60 then bounces 3 months",
        "source": "FRED MICH; yfinance",
