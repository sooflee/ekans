"""PL122_hdd_spike_utilities -- Nat Gas Price Spike -> Long XLU
...
def main():
    sid = "PL122_hdd_spike_utilities"
    
    # Load FRED data
    try:
        fred = load_fred(['MHHNGSP'], start="1990-01-01")
    ...
    for i in range(len(series)):
        if series.iloc[i] < 4:
            was_below = True
        elif was_below and series.iloc[i] >= 4:
    ...
    # Load prices
    try:
        px = load_prices(['XLU', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Nat Gas Price Spike -> Long XLU")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long XLU 126d when MHHNGSP crosses above $4 from below",
        "source": "FRED MHHNGSP; yfinance",
