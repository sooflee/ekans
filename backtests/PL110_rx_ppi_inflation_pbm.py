"""PL110_rx_ppi_inflation_pbm -- Rx PPI Inflation -> Long CI+UNH
Long CI+UNH 126d when PCU325411325411 YoY > +5% for 3 months
"""
import sys
...
def main():
    sid = "PL110_rx_ppi_inflation_pbm"
    
    # Load FRED data
    try:
        fred = load_fred(['PCU325411325411'], start="1990-01-01")
    ...
    series = fred["PCU325411325411"].dropna()
    if len(series) < 13:
    ...
    # Load prices
    try:
        px = load_prices(['CI', 'UNH', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Rx PPI Inflation -> Long CI+UNH")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long CI+UNH 126d when PCU325411325411 YoY > +5% for 3 months",
        "source": "FRED PCU325411325411; yfinance",
