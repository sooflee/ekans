"""PL637_btc_basis_compress_short_mstr — CME BTC Basis Compression -> Short MSTR / Long IBIT Pair"""
import sys
...
def main():
    sid = "PL637_btc_basis_compress_short_mstr"
    tickers = ["MSTR", "IBIT", "BTC=F", "BTC-USD", "SPY"]
    try:
    ...
    m = compute_metrics(pnl, benchmark=spy_r, name="CME BTC Basis Compression -> Short MSTR / Long IBIT Pair", positions=pos)
    save_result(sid, m, extra={
        "rule": "Use yfinance BTC=F (CME front-month BTC futures) and BTC-USD (spot). Compute annualized basis = (BTC=F_close / BTC-USD_close - 1) * (365 / days_to_expiry) \u2014 approximate days_to_expiry as 30 (front-mon",
        "mechanism": "Use yfinance BTC=F (CME front-month BTC futures) and BTC-USD (spot). Compute annualized basis = (BTC=F_close / BTC-USD_close - 1) * (365 / days_to_expiry) \u2014 approximate days_to_expiry as 30 (front-month convention). Use FRED DTB3 (3-month T-bill rate, daily) as carry hurdle. ENTER pair: short MSTR /",
