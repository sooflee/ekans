...
def main():
    sid = "PL111_medtech_spending_recovery"
    ...
    for i in range(len(yoy)):
        if yoy.iloc[i] < 2:
            low_count += 1
            fired = False
        elif low_count >= 6 and yoy.iloc[i] >= 5 and not fired:
    ...
    # Load prices
    try:
        px = load_prices(['SYK', 'ISRG', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Health Store Sales Recovery -> Long SYK+ISRG")
