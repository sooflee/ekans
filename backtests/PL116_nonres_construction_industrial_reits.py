"""PL116_nonres_construction_industrial_reits -- Nonres Construction Inflection -> Long PLD
...
def main():
    sid = "PL116_nonres_construction_industrial_reits"
    
    # Load FRED data
    try:
        fred = load_fred(['TLNRESCONS'], start="1990-01-01")
    ...
    # Load prices
    try:
        px = load_prices(['PLD', 'SPY'], start="1995-01-01")
    except Exception as e:
    ...
    m = compute_metrics(in_pos, benchmark=spy_ret, name="Nonres Construction Inflection -> Long PLD")
    rets_arr = [e["return"] for e in event_results]
    
    save_result(sid, m, extra={
        "rule": "Long PLD 252d when TLNRESCONS YoY turns positive after 6+ months negative",
        "source": "FRED TLNRESCONS; yfinance",
